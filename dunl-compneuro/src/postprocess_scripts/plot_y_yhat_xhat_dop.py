...
def init_params():
    ...
    parser.add_argument(
        ...
        type=str,
        help="res path",
        default= "dopamine/results/dopamine_photometry_numwindow1_roi2_kernellength30_1kernels_1000unroll_2025_02_20_15_49_52"
    )
    parser.add_argument(
        "--sampling-rate",
        type=int,
...
def plot_y_yhat_xhat(y, yhat, xhat, roi):

    i = 0
    #j = 16

    yi = y[i, roi, :].clone().detach().cpu().numpy()
    yihat = yhat[i, roi, :].clone().detach().cpu().numpy()[0]
    codehat = xhat[i, roi, :].clone().detach().cpu().numpy()[0]
...
def main():
    ...
    plot_y_yhat_xhat(y, yhat, xhat, params["roi"])
