...
def init_params():
    ...
    parser.add_argument(
        ...
        help="res path",
        default=[
            #f"dopamine/results/dopamine_photometry_numwindow1_neuron{i}_kernellength30_1kernels_1000unroll_2025_02_05_22_53_39" 
            f"dopamine/results/dopamine_photometry_numwindow1_roi{i}_kernellength30_1kernels_1000unroll_2025_02_20_15_49_52"
            for i in range(19)
        ]  
...
def main():
    ...
    for idx, res_path in enumerate(params_init["res_path"]):
        ...
        plot_kernel(kernels, params, out_path, idx)


def plot_kernel(kernels, params, out_path, idx):
    axes_fontsize = 15
    legend_fontsize = 8
    tick_fontsize = 15
    title_fontsize = 20
    ...
    plt.savefig(
        os.path.join(out_path, f"kernel_{idx}.png"),
        bbox_inches="tight",
        pad_inches=0.02,
    )
    plt.close()
